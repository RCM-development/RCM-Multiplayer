...
namespace RCM_Coop.Network.Entities{
    ...
    public class EntitySerializer{
        public class entity_state{
            ...
            public byte child_controller_index;
        ...
        public static EntityController DecompileEntity(entity_state state, bool no_skipping){
            ...
            if (state.child_controller_index == 255){
                try{
                    ...
                    else result = Patch_InstantiateEntity_Stub.PrefixedOriginal(
                            state.entity_id,
                            state.pos,
                            parent,
                            state.TagFromEnum(),
            ...
            else
            {
                if (parent != null)
                {
                    // register entity controller into networked entities
                    if (parent.childEntityControllers.Count > state.child_controller_index)
                    {
                        RCMManager.Log($"[Co-op] successfully found parent for embedded entity controller");
                        EntityController entity = parent.childEntityControllers[state.child_controller_index];
                        if (entity != null)
                        {
                            NetworkedEntities.InsertEntity(entity, state.owning_player, state.child_controller_index, state.network_id);
                        }
                        else
                        {
                            RCMManager.Log($"[Co-op] WARNING: indexed into null child entity controller from parent... index: {state.child_controller_index}, total child controllers: {parent.childEntityControllers.Count}");
                        }
                        result = entity;
                    }
                    else
                    {
                        RCMManager.Log($"[Co-op] WARNING: failed bad index for parent for embedded entity controller {state.child_controller_index}, total child controllers: {parent.childEntityControllers.Count}");
                    }
                }
                else
                {
                    RCMManager.Log($"[Co-op] no parent for child entity ID thing !!!! child_controller_index:{state.child_controller_index}, parent_networkid:{state.parent_controller_id}");
                }
            }
            return result;
        }
        ...
        public static entity_state CompileEntity(EntityController entity, ushort id, byte owning_player, byte child_controller_index, entity_state state) {
            ...
            state.child_controller_index = child_controller_index;
            if (child_controller_index == 255){
            ...
            {
                state.child_controller_index = child_controller_index;
                if (entity.OriginEntity != null)
                    state.parent_controller_id = NetworkedEntities.IdFromEntity(entity.OriginEntity);
                else state.parent_controller_id = (ushort)0xffff;
                state.parent_transform_id = 0xffff;
                state.parent_transform_index = 0xffff;
            }
        ...
        public static List<entity_state> CompileEntities(){
            ...
            foreach (var entity_struct in NetworkedEntities.IterateNetworkedEntities()){
                ...
                entities.Add(CompileEntity(entity_struct.Value.entity, (ushort)id, entity_struct.Value.owning_player, entity_struct.Value.child_entity_index, new entity_state()));
            }
